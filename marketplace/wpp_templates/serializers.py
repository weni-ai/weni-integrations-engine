...
class TemplateTranslationSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        # update TemplateTranslation instance fields
        instance.body = validated_data.get('body', {}).get('text', instance.body)
        instance.footer = validated_data.get('footer', {}).get('text', instance.footer)
        instance.language = validated_data.get('language', instance.language)
        instance.country = validated_data.get('country', instance.country)
        #instance.status = validated_data.get('status', instance.status)

        components = [instance.body, instance.footer, instance.language, instance.country]

        # update related objects
        header_data = validated_data.get('header')
        if header_data:
            header, _ = TemplateHeader.objects.get_or_create(
                translation=instance, defaults=header_data
            )
            header.update(**header_data)

        buttons_data = validated_data.get('buttons')
        if buttons_data is not None:
            instance.buttons.all().delete()
            buttons = [
                TemplateButton(translation=instance, **button_data)
                for button_data in buttons_data
            ]
            TemplateButton.objects.bulk_create(buttons)

        template_message_request.update_template_message(
