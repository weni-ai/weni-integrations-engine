...
class SetUpService(PermissionTestCaseMixin, APIBaseTestCase):
...
class CreateVtexAppTestCase(SetUpService):
    ...
    def test_create_app_without_valid_wpp_cloud_app(self):
        invalid_app = App.objects.create(
            code="wpp",
            created_by=self.user,
            project_uuid=self.project_uuid,
            platform=App.PLATFORM_WENI_FLOWS,
        ...
        body = self.body.copy()
        body["wpp_cloud_uuid"] = str(invalid_app.uuid)

        response = self.request.post(self.url, body)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "does not correspond to a valid 'wpp-cloud'",
...
class DeleteVtexAppTestCase(SetUpService):
    ...
    def test_delete_app_with_internal_permission_only(self):
        self.user_authorization.delete()
        self.grant_permission(self.user, "can_communicate_internally")
        response = self.request.delete(self.url, uuid=self.app.uuid)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_delete_app_without_permission_and_authorization(self):
        self.user_authorization.delete()
        self.clear_permissions(self.user)
        response = self.request.delete(self.url, uuid=self.app.uuid)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
