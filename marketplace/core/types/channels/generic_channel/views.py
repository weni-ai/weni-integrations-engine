...
class GenericChannelViewSet(views.BaseAppTypeViewSet):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(code=self.kwargs.get("code"))

    def perform_create(self, serializer):
        channel_code = self.kwargs.get("code", None)
        if channel_code:
            channel_code = self.kwargs.get("code").strip()

        if not channel_code:
            raise serializers.ValidationError('Code not be empty.')

        instance = serializer.save(code="generic")
        instance.code = channel_code
    ...
    def configure(self, request, **kwargs):
        ...
        data = request.data
        data["code"] = self.kwargs.get("code", None)
