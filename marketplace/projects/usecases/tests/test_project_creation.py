...
class ProjectCreationTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.project = Project.objects.create(
            uuid=uuid.uuid4(), name="Test Project", created_by=self.user
        )
    ...
    def test_set_user_project_authorization_role_create_new_project_authorization(self):
        usecase = ProjectCreationUseCase(self.template_type_integration)
        usecase.set_user_project_authorization_role(
            self.user, self.project, ProjectAuthorization.ROLE_ADMIN
        )

        project_auth = ProjectAuthorization.objects.get(
            project_uuid=self.project.uuid, user=self.user
        )
        self.assertEquals(project_auth.role, ProjectAuthorization.ROLE_ADMIN)

    def test_project_authorization_already_exists_is_updated_on_set_user_project_authorization_role(
        self,
    ):
        ProjectAuthorization.objects.create(
            user=self.user, project_uuid=self.project.uuid, role=1
        )
    ...
    def test_create_project_sets_created_user_permission_equal_to_admin(self):
        ...
        project_auth = ProjectAuthorization.objects.get(
            project_uuid=project_uuid, user=self.user
        )
        self.assertEquals(project_auth.role, ProjectAuthorization.ROLE_ADMIN)
    ...
    def test_set_users_project_authorizations_creates_object_with_rules(self):
        ...
        for authorization in authorizations:
            db_authorization = ProjectAuthorization.objects.get(
                user__email=authorization.get("user_email"),
    ...
    def test_set_users_project_authorizations_ignores_invalid_roles_and_emails(self):
        ...
        project_authorizations = ProjectAuthorization.objects.filter(
            project_uuid=self.project.uuid
        )
        ...
        self.assertTrue(
            project_authorizations.filter(
                user__email="userrole3@marketplace.ai"
            ).exists()
        )
        self.assertTrue(
            project_authorizations.filter(
                user__email="userrole2@marketplace.ai"
            ).exists()
        )

        self.assertFalse(
            project_authorizations.filter(
                user__email="userignore1@marketplace.ai"
            ).exists()
        )
        self.assertFalse(
            project_authorizations.filter(
                user__email="userignore2@marketplace.ai"
            ).exists()
        )
