...
class TemplateTypeIntegrationTestCase(TestCase):
    ...
    def test_project_already_has_template_raises_invalid_template_type_data(self):
        ...
        template_type = TemplateType.objects.create(
            uuid=template_type_uuid, name="Fake Template Type"
        )
        ...
        with self.assertRaisesMessage(
            ...
            f"The project `{self.project.uuid}` already has an integrated template!",
        ):
            usecase.integrate_template_type_in_project(
                self.project, template_type_uuid, self.user
            )

    def test_send_template_type_uuid_equal_to_none_raises_invalid_template_type_data(
        self,
    ):
        usecase = TemplateTypeIntegrationUseCase(self.app_setup_handler)
        with self.assertRaisesMessage(
    ...
    def test_sending_uuid_template_type_does_not_exist_raises_invalid_template_type_data(
        self,
    ):
        template_type_uuid = uuid.uuid4()
        usecase = TemplateTypeIntegrationUseCase(self.app_setup_handler)
        with self.assertRaisesMessage(
            InvalidTemplateTypeData,
            f"Template Type with uuid `{template_type_uuid}` does not exists!",
        ):
            usecase.integrate_template_type_in_project(
                self.project, template_type_uuid, self.user
            )
    ...
    def test_setup_apps_in_project_called_once(self):
        template_type = TemplateType.objects.create(
            uuid=uuid.uuid4(), name="Fake Template Type"
        )

        usecase = TemplateTypeIntegrationUseCase(self.app_setup_handler)
        usecase.integrate_template_type_in_project(
            self.project, template_type.uuid, self.user
        )
